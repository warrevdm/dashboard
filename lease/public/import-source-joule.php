...
$incomingFile = __DIR__ . '/../storage/incoming/joule-delivered.csv';
$uploadDir = __DIR__ . '/../storage/uploads/';

if (!file_exists($incomingFile)) {
    die('Geen Joule-scrape gevonden. Run eerst de Joule scraper zodat storage/incoming/joule-delivered.csv bestaat.');
}
...
if (!copy($incomingFile, $targetPath)) {
    die('Kon Joule-bestand niet kopiëren naar uploads.');
}
...
if (!$template) {
    die('Geen mappingtemplate “joule” gevonden. Maak eerst een mappingtemplate met naam joule.');
}

$mapping = json_decode($template['mapping_json'], true);

if (!is_array($mapping)) {
    die('Mappingtemplate “joule” bevat geen geldige mapping.');
}
...
<head>
    <meta charset="UTF-8">
    <title>Joule import voorbereiden | Lease Import Manager</title>
    <link rel="stylesheet" href="assets.css">
</head>
<body>

<header class="page-header">
    <h1>Joule import voorbereiden</h1>
...
<main>
    <section class="card">
        <h2>Laatste Joule-scrape gevonden</h2>

        <p><strong>Bronbestand:</strong> storage/incoming/joule-delivered.csv</p>
    ...
    <section class="card">
        <form action="import-review.php" method="POST">
            ...
            <button type="submit">Preview Joule-import bekijken</button>
